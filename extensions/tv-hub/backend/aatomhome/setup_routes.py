...
def _checklist(ha_cfg: dict[str, str], ha_test: dict[str, Any], tvs: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    items: list[dict[str, Any]] = [
        ...
        {
            "id": "tv_registered",
            "label": "At least one TV registered",
            "status": "ok" if tvs.get("registered") else "todo",
            "detail": f"{tvs.get('registered', 0)} registered",
        },
        {
            "id": "tv_online",
            "label": "TV connected over ADB",
            "status": "ok" if tvs.get("online") else ("warn" if tvs.get("registered") else "todo"),
            "detail": f"{tvs.get('online', 0)} online",
        },
    ]
    if tvs.get("pending"):
        items.append(
