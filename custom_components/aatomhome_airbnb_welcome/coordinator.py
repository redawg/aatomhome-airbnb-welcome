...
class TvHubCoordinator(DataUpdateCoordinator[TvHubData]):
    ...
    async def _async_update_data(self) -> TvHubData:
        ...
        try:
            from .plugins import run_sync_hooks

            await run_sync_hooks(
                self,
                {
                    "health": health,
                    "registry": filtered_registry,
        ...
        except Exception:  # noqa: BLE001 — plugins must not break core poll
            _LOGGER.debug("Plugin sync hooks skipped or failed", exc_info=True)
        return data
